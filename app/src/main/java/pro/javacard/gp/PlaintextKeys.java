...
public class PlaintextKeys extends GPCardKeys {
    ...
    // After diversify() we know for which protocol we have keys for, unless known
    // before
    ...
    static byte[] validateKey(byte[] k) {
        if (k.length != 16 && k.length != 24 && k.length != 32) {
            throw new IllegalArgumentException(
                    String.format("Invalid key length %d: %s", k.length, HexUtils.bin2hex(k)));
        }
        return k;
    }

    public static Optional<PlaintextKeys> fromBytes(byte[] enc, byte[] mac, byte[] dek, byte[] mk, KDF kdf, byte[] kdd,
            int ver) {
        if ((enc != null || mac != null || dek != null) && (enc == null || mac == null || dek == null || mk != null)) {
            throw new IllegalArgumentException(
                    "Either all or nothing of enc/mac/dek keys must be set, and no mk at the same time!");
        }
        ...
        if (enc != null && mac != null && dek != null) {
            ...
            // If the actual KDD does not match for some reason what is returned by the
            // card, allow for easy override
            if (kdd != null) {
                keys.kdd = kdd.clone();
            }
            if (ver != 0) {
    ...
    public static Optional<PlaintextKeys> fromStrings(String enc, String mac, String dek, String mk, String div,
            String kdd, String ver) {
        if ((enc != null || mac != null || dek != null) && (enc == null || mac == null || dek == null || mk != null)) {
            throw new IllegalArgumentException(
                    "Either all or nothing of enc/mac/dek keys must be set, and no mk at the same time!");
        }
    ...
    // Returns empty if no variables present, throws illegal argument if variable
    // invalid
    ...
    public static byte[] diversify(byte[] k, KeyPurpose usage, byte[] kdd, KDF method) throws GPException {
        try {
            final byte[] kv;
            if (method == KDF.KDF3) {
                return GPCrypto.scp03_kdf(k, new byte[] {}, GPUtils.concatenate(SCP03_KDF_CONSTANTS.get(usage), kdd),
                        k.length);
            } else {
    ...
    public byte[] encryptKey(GPCardKeys key, KeyPurpose p, byte[] sessionContext) throws GeneralSecurityException {
        ...
        switch (scp) {
            case SCP01:
                logger.debug("Encrypting {} value (KCV={}) with DEK (KCV={})", p, HexUtils.bin2hex(other.kcv(p)),
                        HexUtils.bin2hex(kcv(KeyPurpose.DEK)));
                return GPCrypto.dek_encrypt_des(cardKeys.get(KeyPurpose.DEK), other.cardKeys.get(p));
            case SCP02:
                byte[] sdek = deriveSessionKeySCP02(cardKeys.get(KeyPurpose.DEK), KeyPurpose.DEK, sessionContext);
                logger.debug("Encrypting {} value (KCV={}) with S-DEK (KCV={})", p, HexUtils.bin2hex(other.kcv(p)),
                        HexUtils.bin2hex(GPCrypto.kcv_3des(sdek)));
    ...
    public String toString() {
        ...
        return String.format("ENC=%s (KCV: %s) MAC=%s (KCV: %s) DEK=%s (KCV: %s) for %s%s", enc, enc_kcv, mac, mac_kcv,
                dek, dek_kcv, scp, kdf == null ? "" : String.format(" with %s", kdf));
    }
